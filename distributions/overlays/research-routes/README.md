...
研究 Pack runtime 位于 `plugins/research-routes/runtime/research_routes_runtime/`，版本为 `0.3.0-rc.1`，只使用 Core 的通用 artifact、snapshot、decision 和 promotion 边界。terrain map 不选择路线，也不授权 experiment、implementation 或 canonical state；route authorization 必须引用 confirmed HumanDecision，canonical promotion 还需独立的 `canonical-promotion` decision。分发包从唯一 canonical `allinluna_runtime` 加载宿主运行时，并同步研究 Pack runtime。分发验收报告区分 `requested`、`resolved`、`actual` 资源值，并使用 schema v8、route assurance 与 exact relay 术语。`identity`、`create`、`read`、`wait`、`cancel` 与 `idempotency` 都完整时为 `PASS`；缺失或阻塞的证据为 `BLOCKED`。
