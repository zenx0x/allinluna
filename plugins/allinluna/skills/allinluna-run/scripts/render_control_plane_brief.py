#!/usr/bin/env python3
"""Render self-contained briefs for the primary and child coordinators."""
...
must not implement product files. Load `{run_dir}/run-state.json`, validate it, then run
coordinator ticks, create child coordinators or owners, monitor available host threads, and stop
only at the recorded boundary or a true sponsor decision. Send only material choices,
authorization requests, and milestone summaries to the sponsor.
...
dependencies, ownership conflicts, resource exhaustion, or true blockers to the primary.
...
def main() -> int:
    ...
    parser.add_argument("--role", choices=["primary-coordinator", "subcoordinator"], required=True)
