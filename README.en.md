...
Model and reasoning choices come from the Run resource policy and may be overridden by a narrower Task or WorkUnit policy; Luna-high is no longer hard locked. Callers may select Luna medium/high/xhigh/max, Codex Spark, or another host-supported model. Keep requested, resolved, and actual evidence separate. Actual is proven by App Server `thread/start`, `model/rerouted`, and turn lifecycle events exported by the Desktop host, never inferred from task prose or accepted from a separate CLI App Server session. The outer receipt still identifies Desktop `codex_app__create_thread`; absent evidence remains `unresolved`.

A real host receipt must return `resource_receipt.requested/resolved/actual`, `actual_state`, `evidence_source`, `observed_at`, and its App Server route evidence. Requested must match the persisted dispatch action; actual must match the final resolved route. Requested may differ from resolved only when an unbroken `model/rerouted` chain proves the change. Missing fields, invalid timestamps, a missing action baseline, or incoherent events remain `unresolved`. `runtime.db` schema v5 persists all three resource pairs for crash recovery, replay, and status queries.
