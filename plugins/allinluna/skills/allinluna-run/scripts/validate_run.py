#!/usr/bin/env python3
"""Validate the lean All in Luna recovery snapshot."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any

from runtime_truth import assignment_conflicts, runtime_identity_errors, task_contract_errors
from workflow_state import (
    RUN_TRANSITIONS,
    TASK_TRANSITIONS,
    dependencies_satisfied,
    hard_lock_family,
    json_sha256,
    load_state,
    model_matches_lock,
    read_json,
    resolve_topology,
)


REQUIRED_STATE = {
    "schema_version",
    "run_id",
    "plan_id",
    "plan_hash",
    "execution_style",
    "risk_level",
    "topology",
    "run_dir",
    "status",
    "profile",
    "goal_authorized",
    "capabilities",
    "resource_policy",
    "workflow_preset",
    "usage",
    "repository",
    "authorizations",
    "orchestration",
    "control_plane",
    "coordination",
    "completion_standard",
    "tasks",
    "milestones",
    "created_at",
    "updated_at",
}


def _budget_value(state: dict[str, Any]) -> tuple[str | None, float | None, float | None]:
    budget = state.get("resource_policy", {}).get("budget", {})
    usage = state.get("usage", {})
    metric = budget.get("metric")
    if metric == "tokens":
        used = usage.get("tokens")
    elif metric == "credits":
        used = usage.get("credits")
    elif metric == "time-minutes":
        seconds = usage.get("elapsed_seconds")
        used = seconds / 60 if isinstance(seconds, (int, float)) else None
    elif metric == "currency":
        used = usage.get("currency")
    else:
        used = None
    return metric, float(used) if isinstance(used, (int, float)) else None, budget.get("hard_limit")


def validate(target: Path) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    try:
        run_dir, state = load_state(target)
    except (OSError, json.JSONDecodeError) as exc:
        return {"valid": False, "errors": [str(exc)], "warnings": []}

    missing = sorted(REQUIRED_STATE - state.keys())
    if missing:
        errors.append("missing state fields: " + ", ".join(missing))
    if state.get("schema_version") != "2.0":
        errors.append("schema_version must be 2.0")
    if Path(state.get("run_dir", "")).resolve() != run_dir.resolve():
        errors.append("run_dir in state does not match the actual directory")
    if state.get("status") not in RUN_TRANSITIONS:
        errors.append(f"invalid run status: {state.get('status')}")

    capabilities = state.get("capabilities", {})
    if capabilities.get("actual_delegation") == "top-level-task" and not state.get("authorizations", {}).get("top_level_tasks"):
        errors.append("run records top-level-task delegation without authorization")
    host_concurrency = capabilities.get("host_concurrency")
    if isinstance(host_concurrency, int) and host_concurrency < 1:
        errors.append("host_concurrency must be positive")
    orchestration = state.get("orchestration", {})
    if orchestration.get("sponsor_role") != "user-conversation":
        errors.append("user conversation must remain the sponsor")
    if orchestration.get("coordinator_role") != "separate-top-level-task":
        errors.append("primary coordinator must remain a separate top-level task")
    if orchestration.get("coordinator_product_implementation") != "forbidden":
        errors.append("coordinator product implementation must remain forbidden")
    if "counterpilot" in orchestration or "acceptance" in state:
        errors.append("lean runtime state must not materialize CounterPilot or Acceptance")
    control = state.get("control_plane", {})
    sponsor = control.get("sponsor", {})
    primary = control.get("primary_coordinator", {})
    if sponsor.get("thread_id") and sponsor.get("thread_id") == primary.get("thread_id"):
        errors.append("sponsor and primary coordinator must use different threads")
    for name, item in control.get("subcoordinators", {}).items():
        if item.get("thread_id") in {sponsor.get("thread_id"), primary.get("thread_id")} and item.get("thread_id"):
            errors.append(f"{name} must use an independent thread")

    plan_path = run_dir / "plan.json"
    if not plan_path.exists():
        errors.append("plan.json snapshot is missing")
        plan = None
    else:
        try:
            plan = read_json(plan_path)
            if json_sha256(plan) != state.get("plan_hash"):
                errors.append("plan.json hash does not match plan_hash")
            if plan.get("plan_id") != state.get("plan_id"):
                errors.append("plan_id differs between plan snapshot and run state")
        except (OSError, json.JSONDecodeError) as exc:
            errors.append(f"cannot read plan.json: {exc}")
            plan = None
    if plan is not None:
        expected = resolve_topology(plan)
        if state.get("topology", {}).get("resolved") != expected.get("resolved"):
            errors.append("run topology projection does not match the plan")
        errors.extend(task_contract_errors(state, plan))

    tasks = state.get("tasks")
    if not isinstance(tasks, dict) or not tasks:
        errors.append("tasks must be a non-empty object")
        tasks = {}
    lock = hard_lock_family(state)
    if state.get("profile") in {"all-luna", "mad-luna"} and (not lock or "luna" not in lock.casefold()):
        errors.append(f"{state.get('profile')} run is missing its Luna hard lock")
    for task_id, task in tasks.items():
        prefix = f"task {task_id}"
        if task.get("id") != task_id:
            errors.append(f"{prefix} id does not match map key")
        status = task.get("status")
        if status not in TASK_TRANSITIONS:
            errors.append(f"{prefix} has invalid status {status}")
            continue
        dependencies = task.get("dependencies")
        if not isinstance(dependencies, list):
            errors.append(f"{prefix} dependencies must be an array")
        else:
            for dependency in dependencies:
                if dependency not in tasks:
                    errors.append(f"{prefix} references missing dependency {dependency}")
        if status in {"ready", "running", "completed"} and not dependencies_satisfied(task, tasks):
            errors.append(f"{prefix} is {status} with incomplete dependencies")
        evidence = task.get("evidence", {})
        if status == "completed":
            if not evidence.get("checks"):
                errors.append(f"{prefix} is completed without check evidence")
            if task.get("ownership", {}).get("paths") and state.get("authorizations", {}).get("git_operations") and not evidence.get("final_commit"):
                errors.append(f"{prefix} is completed without a final commit")
        if status == "skipped" and not evidence.get("skip_approved"):
            errors.append(f"{prefix} is skipped without explicit approval")
        actual_model = task.get("actual", {}).get("model", "unavailable")
        if lock and actual_model != "unavailable" and not model_matches_lock(str(actual_model), lock):
            errors.append(f"{prefix} actual model {actual_model!r} violates hard lock {lock!r}")
        if status == "completed" and actual_model == "unavailable":
            warnings.append(f"{prefix} completed but actual model was not exposed by the host")
        if status in {"running", "completed"}:
            errors.extend(f"{prefix} {item}" for item in runtime_identity_errors(task, state, require_started=True))
        if task.get("resource_class") == "acceptance":
            errors.append(f"{prefix} Acceptance task must not be materialized")
    errors.extend(assignment_conflicts(tasks))

    metric, consumed, hard = _budget_value(state)
    if metric and consumed is not None and isinstance(hard, (int, float)) and consumed >= hard and state.get("status") in {"planned", "running"}:
        errors.append(f"{metric} hard budget reached but run is not paused: {consumed} >= {hard}")

    if state.get("status") == "completed":
        incomplete = [task_id for task_id, task in tasks.items() if task.get("status") not in {"completed", "skipped"}]
        if incomplete:
            errors.append("completed run has incomplete tasks: " + ", ".join(incomplete))
    counts: dict[str, int] = {}
    for task in tasks.values():
        counts[task.get("status", "invalid")] = counts.get(task.get("status", "invalid"), 0) + 1
    return {
        "valid": not errors,
        "errors": list(dict.fromkeys(errors)),
        "warnings": list(dict.fromkeys(warnings)),
        "summary": {"run_id": state.get("run_id"), "status": state.get("status"), "profile": state.get("profile"), "tasks": counts},
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("run", type=Path)
    parser.add_argument("--pretty", action="store_true")
    args = parser.parse_args(argv)
    result = validate(args.run)
    print(json.dumps(result, indent=2 if args.pretty else None, ensure_ascii=False))
    return 0 if result["valid"] else 1


if __name__ == "__main__":
    raise SystemExit(main())
